...
class TicketService:
    ...
    def create_ticket(self, title: str, description: str, **kwargs) -> str:
        """Create a ticket with enhanced routing logic"""
        ...
        logger.info(f"Calculated importance score: {importance}/10")
        ...
        if target_location == "notification_only":
            logger.info(f"Issue deemed common/low priority - sending notification only")
        ...
        if target_location == "sprint":
            ...
            logger.info(f"Adding ticket {ticket_id} to sprint based on decision type: {decision_type}")
            self._add_issue_to_sprint(ticket_id, sprint_name)
        elif target_location == "backlog":
            logger.info(f"Adding ticket {ticket_id} to backlog based on decision type: {decision_type}")
            self._ensure_issue_in_backlog(ticket_id)
        
        return ticket_id
    
    def _calculate_importance(self, title: str, description: str, **kwargs) -> int:
        """
        Calculate importance score (0-10) based on severity, impact, and content
        """
        ...
        text = (title + " " + description).lower()
        urgent_terms = ["urgent", "critical", "outage", "broken", "crash", "failure", 
                    "security", "vulnerability", "breach", "data loss", "production"]
        
        # Add points for urgent terms
        for term in urgent_terms:
            if term in text:
                score += 1
                # Don't add more than 3 points for urgent terms
                if score > 8:
                    break
    ...
    def _determine_ticket_location(self, decision_type: str, importance: int) -> str:
        ...
        if decision_type == "fix_code":
            ...
            # Investigation tickets go to backlog unless very important
            return "sprint" if importance >= 8 else "backlog"
        
        elif decision_type == "add_ticket":
            # Regular tickets are routed based on importance
            return "sprint" if importance >= 7 else "backlog"
        
        elif decision_type == "notification_only" or importance <= 3:
            # Very low importance issues just get a notification
